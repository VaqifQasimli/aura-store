cat > /mnt/user-data/outputs/backend.py << 'ENDOFFILE'
...
import hashlib, os, smtplib, urllib.request, urllib.parse, time, json
...
CLOUDINARY_KEY_VAR = os.getenv("CLOUDINARY_API_KEY", "")
...
def product_dict(p):
    return {
        ...
        "badge": p.badge, "stock": p.stock, "active": p.active,
    ...
    <div style="text-align:right;margin-top:20px;font-size:22px;font-weight:700;color:#C9A227">
      Ümumi: {order.total_amount:.2f} ₼
        ...
        payload = json.dumps({
            ...
            "subject": f"Yeni Sifaris {order.order_number} — AURA Store",
            "html": html
        }).encode()
        ...
        print(f"Email gonderildi: {order.order_number}")
    except Exception as e:
        print(f"Email xetasi: {e}")
...
@app.get("/")
def root():
    return {"app": "AURA Store API", "version": "2.0.0", "status": "Isleyir"}

@app.post("/api/auth/register")
def register(data: RegisterSchema, db: Session = Depends(get_db)):
    if len(data.name.strip()) < 2:
        raise HTTPException(400, "Ad en az 2 herf olmalidir")
    if len(data.password) < 6:
        raise HTTPException(400, "Sifre en az 6 simvol olmalidir")
    if db.query(User).filter(User.email == data.email.lower().strip()).first():
        raise HTTPException(400, "Bu email artiq qeydiyyatdadir")
    user = User(
...
def login(data: LoginSchema, db: Session = Depends(get_db)):
    ...
    if not user or not verify_pw(data.password, user.hashed_password):
        raise HTTPException(401, "Email ve ya sifre yanlisdir")
    return {
        "success": True,
        "token": create_token(user.id),
        "refresh_token": create_refresh_token(user.id),
...
def refresh(data: RefreshSchema, db: Session = Depends(get_db)):
    ...
    user = db.query(User).filter(User.id == user_id, User.is_active == True).first()
    if not user:
        raise HTTPException(401, "Istifadeci tapilamdi")
    return {
        "success": True,
        "token": create_token(user.id),
        "refresh_token": create_refresh_token(user.id),
...
def admin_login(data: AdminLoginSchema):
    if data.password != ADMIN_PASSWORD:
        raise HTTPException(401, "Admin sifresi yanlisdir")
...
def admin_update_product(pid: int, data: ProductSchema, is_admin: bool = Depends(get_admin), db: Session = Depends(get_db)):
    ...
    if not p:
        raise HTTPException(404, "Mehsul tapilamdi")
    d = data.dict()
    imgs = d.pop("images", [])
    for k, v in d.items():
        setattr(p, k, v)
...
async def upload_image(request: Request, is_admin: bool = Depends(get_admin)):
    if not CLOUDINARY_CLOUD or not CLOUDINARY_KEY_VAR or not CLOUDINARY_SEC:
        raise HTTPException(400, "Cloudinary konfiqurasiya edilmeyib")
    body = await request.json()
    image_data = body.get("image")
    if not image_data:
        raise HTTPException(400, "Sekil tapilamdi")
...
def admin_update_order(oid: int, data: OrderStatusSchema, is_admin: bool = Depends(get_admin), db: Session = Depends(get_db)):
    ...
    if not o:
        raise HTTPException(404, "Sifaris tapilamdi")
...
def create_order(data: OrderSchema, current_user: User = Depends(get_user), db: Session = Depends(get_db)):
    ...
    for ci in data.items:
        ...
        if not p:
            raise HTTPException(404, f"Mehsul tapilamdi: {ci.product_id}")
        if p.stock < ci.quantity:
            raise HTTPException(400, f"'{p.name}' anbarda yoxdur")
    ...
    return {
        ...
        "message": "Sifarisинiz qebul edildi!"
    }
    ...
    return [
        {
            "id": o.id, "order_number": o.order_number,
            "total_amount": o.total_amount, "status": o.status,
            "city": o.city, "payment_method": o.payment_method,
            "created_at": o.created_at.isoformat(),
            "items": [
                {"name": i.product.name, "emoji": i.product.emoji,
                 "qty": i.quantity, "price": i.unit_price}
...
def seed():
    ...
    try:
        if db.query(Product).count() > 0:
            return
        items = [
            ("Klassik Koynak","geyim",25,35,"👔",4.5,128,"Endirim",50,"Premium pambiqi klassik koynak"),
            ("Casual T-shirt","geyim",15,None,"👕",4.2,89,"Yeni",80,"Rahat gundalik t-shirt"),
            ("Kisi Kostyum","geyim",180,220,"🤵",4.8,56,"Endirim",15,"Elegant kisi kostyumu"),
            ("Qadin Paltari","geyim",55,None,"👗",4.6,203,None,40,"Sik qadin paltari"),
            ("Deri Ayaqqabi","geyim",95,120,"👞",4.4,77,"Endirim",25,"Genuine deri ayaqqabi"),
            ("Godekce","geyim",140,170,"🧥",4.3,45,None,20,"Qis ucun godekce"),
            ("Smartfon Pro Max","elektronika",999,1199,"📱",4.9,312,"Endirim",10,"En son model smartfon"),
            ("Qulaqliq ANC","elektronika",199,None,"🎧",4.7,188,"Yeni",30,"Aktiv sas-kuy sondurme"),
            ("Noutbuk Ultra","elektronika",1450,1700,"💻",4.8,95,"Endirim",8,"Ultra slim noutbuk"),
            ("Smart Saat","elektronika",249,299,"⌚",4.5,143,None,22,"Saglamliq izleme saati"),
            ("Portativ Dinamik","elektronika",85,None,"🔊",4.4,67,"Yeni",35,"360 portativ dinamik"),
            ("Yuxu Desti","ev",65,None,"🛏️",4.6,231,None,18,"Premium ipeksi yuxu desti"),
            ("Dekorativ Yastiq","ev",18,25,"🛋️",4.2,88,"Endirim",60,"Rengareng dekorativ yastiq"),
            ("Aromatik Sam","ev",22,None,"🕯️",4.8,156,"Yeni",45,"El isi aromatik sam"),
            ("Metbex Desti","ev",110,140,"🍳",4.5,74,"Endirim",12,"Tam metbex qab desti"),
            ("Futbol Topu","idman",45,None,"⚽",4.4,63,None,40,"Professional futbol topu"),
            ("Yoga Mati","idman",35,50,"🧘",4.7,119,"Endirim",28,"Non-slip yoga mati"),
            ("Idman Cantasi","idman",55,None,"🎒",4.3,82,"Yeni",33,"Su kecirmayan idman cantasi"),
        ]
        db.add_all([
            Product(
                name=n, category=c, price=pr, old_price=op,
                emoji=e, rating=r, review_count=rc,
                badge=b, stock=s, description=d, images="[]"
            )
            for n, c, pr, op, e, r, rc, b, s, d in items
        ])
        db.commit()
        print(f"✅ {len(items)} mehsul elave edildi")
    finally:
        db.close()
ENDOFFILE
echo "backend.py yazildi"
