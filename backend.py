...
import hashlib, os, secrets, smtplib
...
# ── ŞİFRƏ (sadə SHA256 — heç bir xarici paket lazım deyil) ─────
...
def create_token(user_id: int) -> str:
    ...
    payload = json.dumps({"sub": user_id, "exp": (datetime.utcnow() + timedelta(days=7)).isoformat()})
...
def send_email(order):
    if not GMAIL_APP_PASSWORD:
        print("⚠️ GMAIL_APP_PASSWORD yoxdur")
        return
    try:
        rows = "".join(
        ...
        <div style="max-width:600px;margin:0 auto;background:#0D0D1A;border-radius:16px;overflow:hidden">
          ...
          <div style="padding:30px;color:#EDEDFF">
            ...
            <table style="width:100%;border-collapse:collapse;margin-top:20px">
              <tr style="background:#1A1A35"><th style="padding:10px;text-align:left">Məhsul</th>
              <th style="padding:10px">Miqdar</th><th style="padding:10px">Cəmi</th></tr>
              {rows}
            </table>
...
def get_products(category: str = None, search: str = None, db: Session = Depends(get_db)):
    ...
    return [{"id":p.id,"name":p.name,"category":p.category,"price":p.price,
             ...
             "rating":p.rating,"review_count":p.review_count,"badge":p.badge,"stock":p.stock}
            for p in q.all()]
...
def my_orders(current_user: User = Depends(get_user), db: Session = Depends(get_db)):
    ...
    return [{"id":o.id,"order_number":o.order_number,"total_amount":o.total_amount,
             "status":o.status,"city":o.city,"payment_method":o.payment_method,
             "created_at":o.created_at.isoformat(),
             "items":[{"name":i.product.name,"emoji":i.product.emoji,
                       "qty":i.quantity,"price":i.unit_price} for i in o.items]}
            for o in orders]
...
def seed():
    ...
    try:
        ...
        db.add_all([Product(name=n,category=c,price=pr,old_price=op,emoji=e,rating=r,
                            review_count=rc,badge=b,stock=s,description=d)
